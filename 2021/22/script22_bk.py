...
#matrix = [[[False] * SIZE for _ in range(SIZE)] for _ in range(SIZE)]

#cnt = 0

#for status, ranges in data:
    #newRanges = tuple(constraintRange(dr, RANGE1) for dr in ranges)
    ##print(newRanges)
    ...
    #for x in range(xr[0], xr[1] + 1):
        ...
        #for y in range(yr[0], yr[1] + 1):
            ...
            ##zm1, zm2 = (n - RANGE1[0] for n in zr)
            ##matrix[xm][ym][zm1:zm2+1] = [status] * (zm2 - zm1 + 1)
            #for z in range(zr[0], zr[1] + 1):
                #zm = z - RANGE1[0]
                
                #if matrix[xm][ym][zm] != status:
                    #matrix[xm][ym][zm] = status
                    #cnt += (1 if status else -1)
